...
{
    //TODO: If localization resources are not found, logging does not work.
    ...
    internal sealed class SocketsEventSource : EventSource
    {
        ...
        internal static void Dump(object method, byte[] buffer, int offset, int length)
        {
            ...
            {
                partialBuffer[i] = buffer[i + offset];
            }
            if (method is MethodType)
            {
                s_log.DebugDumpArray((MethodType)method, buffer);
    ...
    Level = EventLevel.Informational, Message = "FinishOperation({0}Async)")]
        internal unsafe void DumpArrayAsync(SocketAsyncOperation method, byte[] buffer)
        {
            if (!s_log.IsEnabled())
            {
                return;
